<template>
  <div>
    ...
    >
      ...
      >
        <span class="content-Breadcrumb">今日上岗</span>
      </div>
...
<!--            </el-form-item>-->
            ...
            <el-form-item label="铲屎官" prop="master">
              <el-select v-model="form.master" :size="componentSize" filterable clearable placeholder="请选择铲屎官">
                <el-option
...
<!--          </el-button>-->
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <span v-if="scope.row.master == '-1'">原住民</span>
            ...
            >
              <template slot-scope="scope">
                ...
                <span v-else-if="scope.row.status == '-1'">原住民</span>
              </template>
            </el-table-column>
            <el-table-column
                label="操作"
...
<!--                        :size="componentBtnSize"-->
<!--                        @click="modifyWorkcat(scope)"-->
<!--                    >编辑-->
<!--                    </el-button>-->
                    <el-popconfirm title="确定送去休息吗？" @confirm="changeToRest(scope)">
                      <el-button
                          type="success"
                          :size="componentBtnSize"
                          slot="reference"
...
<!--                    <el-popconfirm title="确定删除该条数据吗？" @confirm="deleteWorkcat(scope)">-->
<!--                      <el-button-->
<!--                          type="danger"-->
<!--                          :size="componentBtnSize"-->
<!--                          slot="reference"-->
...
export default {
  ...
  methods: {
    ...
    changeToRest(val){
      ...
      this.$axios.changeToRest(val.row).then((res) => {
    ...
    deleteWorkcat(val) {
      if (val.row.id != null && val.row.id != "") {
        this.$axios.deleteWorkcat(val.row).then((res) => {
          if (res.returnCode == "200") {
            this.$notify({
              title: "成功",
              message: "删除成功",
    ...
    queryAllWorkcatList() {
      ...
      this.loading = true;
      this.tableData = [];
      this.$axios.queryAllWorkcatList(params).then(res => {
    ...
    backHome(){
      this.$confirm('确定到点下班了吗？', '提示', {type: 'success'}).then(() => {
        this.$axios.backHome().then(res => {
          if (res.returnCode == "200") {
            this.$notify({
              title: "成功",
              message: "下班咯",
    ...
    deleteAll() {
      if (this.WorkcatList == null || this.WorkcatList.length == 0) {
        this.$message({
          title: "警告",
          message: "请至少选择一只喵送去摆烂",
          type: "warning",
          duration: 1000,
        });
      } else {
        this.$confirm('确定送去摆烂吗？', '提示', {type: 'warning'}).then(() => {
          this.$axios.goHome(this.WorkcatList).then(res => {
            if (res.returnCode == "200") {
              this.$notify({
                title: "成功",
                message: "摆烂成功",
