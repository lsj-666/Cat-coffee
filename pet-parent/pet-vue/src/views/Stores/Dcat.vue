<template>
  <div>
    ...
    >
      ...
      >
        <span class="content-Breadcrumb">猫咪领养</span>
      </div>
...
export default {
  ...
  methods: {
    ...
    queryAllCcatList() {
      ...
      this.loading = true;
      this.tableData = [];
      this.$axios.queryAllDcatList(params).then(res => {
    ...
    getVendorId(val) {
      // this.selectUsername=val.label;//获取label
      console.log("123123123:"+ this.selectUsername)
      console.log("456456456:"+ val)
    },
    insertSubmit() {
      if (this.title == "新增宠物") {
        ...
        this.$confirm('确定该铲屎官将领养这只小猫吗？', '提示', {type: 'warning'}).then(() => {
