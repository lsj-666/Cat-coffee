package com.example.petparent.config.interceptor;
...
import com.example.petparent.config.AuthAccess;
...
public class JwtInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        if (StrUtil.isBlank(token)) {
            throw new ServiceException(Constants.CODE_401, "token不存在或token已过期，请重新登录");
        }
        // 获取 token 中的 user id
        String userId;
        try {
